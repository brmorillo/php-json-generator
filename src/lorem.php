...
class Lorem
{
    ...
    public function getLoremWords(string $lorem, int $length = 1)
    {
        $palavras = explode(' ', $lorem);
        $primeirasPalavras = array_slice($palavras, $this->number->getInteger(1, count($palavras)), $length);
        return ucfirst(strtolower(str_replace(',', '', str_replace('.', '', implode(' ', $primeirasPalavras)))));
    }

    public function getLoremSentense(string $lorem, int $length = 1)
    {
        $palavras = explode('.', $lorem);
        $primeirasPalavras = array_slice($palavras, $this->number->getInteger(1, count($palavras)), $length);
    ...
    public function getLoremParagraph(string $lorem, int $length = 1)
    {
        $primeirosParagrafos = [];
        $paragrafos = explode('.', $lorem);
        for ($i = 1; $i <= $length; $i++) {
            for ($j = 1; $j <= 4; $j++) {
                $primeirosParagrafos = array_merge(array_slice($paragrafos, $this->number->getInteger(1, count($paragrafos)), $length), $primeirosParagrafos);
            }
        }
        return implode('.', $primeirosParagrafos);
    }

    public function getLorem(int $length = 1, string $type = 'words')
    {
        return 'Lorem Ipsum';
        ...
        if ($type === 'sentenses') {
            $lorem = $this->getLoremSentense($lorem, $length);
        } elseif ($type === 'paragraphs') {
            $lorem = $this->getLoremParagraph($lorem, $length);
        } else {
            $lorem = $this->getLoremWords($lorem, $length);
        }
        return trim($lorem);
    }
}
